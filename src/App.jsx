...
// 노션 API가 CORS 이슈로 브라우저에서 직접 호출이 안 될 수 있어서
// 로컬 데이터를 fallback으로 사용
const FALLBACK_DATA = [
  ...
  { name: "테니스팔찌 잠금 리뉴얼", stage: "드롭", manager: "조안나", dday: "", desc: "- 3프롱\n2.0/2.5/1부 원본 완료 → 금샘플 진행중\n2부/3부 완료\n\n- 4프롱\n2.0/2.5 출력완료→원본완료\n2.8/1부 원본완료 → 금샘플진행중\n2부/3부 완료", updated: "2026-03-09", ref: "진행 보류", collection: "에센셜" },
...
function Card({ item, onClick }) {
  ...
  return (
    <div onClick={() => onClick(item)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ background: hov ? cfg.bg : "#ffffff", border: `1px solid ${hov ? cfg.border : "#e5e7eb"}`, borderLeft: `3px solid ${cfg.color}`, borderRadius: "0 10px 10px 0", padding: "12px 16px", cursor: "pointer", transition: "all 0.15s", boxShadow: hov ? `0 2px 12px ${cfg.color}18` : "0 1px 2px #0000000a" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: "#111827", lineHeight: 1.4, flex: 1 }}>{item.name}</span>
      ...
      {item.desc && <p style={{ margin: "6px 0 0", fontSize: 11, color: "#6b7280", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{item.desc.length > 100 ? item.desc.slice(0, 100) + "…" : item.desc}</p>}
...
function Modal({ item, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "#00000030", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 16, padding: 28, maxWidth: 520, width: "90%", maxHeight: "80vh", overflowY: "auto", boxShadow: "0 20px 60px #00000018" }}>
        ...
        {item.desc && (
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 13, color: "#374151", lineHeight: 1.8, whiteSpace: "pre-wrap", background: "#f9fafb", borderRadius: 8, padding: "12px 14px", border: "1px solid #f3f4f6" }}>{item.desc}</div>
          </div>
        )}
        {item.ref && (
          <div>
            <p style={{ margin: "0 0 6px", fontSize: 10, color: "#9ca3af", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>참고</p>
            {item.ref.startsWith("http") ? <a href={item.ref} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#2563eb", wordBreak: "break-all" }}>🔗 {item.ref}</a> : <div style={{ fontSize: 13, color: "#374151", background: "#f9fafb", borderRadius: 8, padding: "10px 14px" }}>{item.ref}</div>}
          </div>
        )}
      </div>
    </div>
...
function StageSection({ stage, cfg, items, onSelect }) {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div onClick={() => setOpen(o => !o)} style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", padding: "7px 10px", borderRadius: 8, userSelect: "none", background: open ? cfg.bg : "transparent", marginBottom: open ? 8 : 0, transition: "background 0.15s" }}>
...
export default function Dashboard() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const parsed = json.results.map(parseNotionPage).filter(d => d.name.trim());
      setData(parsed);
      setUsingFallback(false);
      setLastUpdated(new Date());
    } catch (e) {
      // CORS 등 이슈시 로컬 데이터 사용
  ...
  const criCount      = data.filter(d => TEAM_STAGES.cri.includes(d.stage)).length;
  const workshopCount = data.filter(d => TEAM_STAGES.workshop.includes(d.stage)).length;
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "20px 24px 18px" }}>
          <div style={{ maxWidth: 860, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
              ...
              <button onClick={fetchData} disabled={loading} style={{ fontSize: 11, padding: "6px 12px", borderRadius: 8, border: "1px solid #e5e7eb", background: "#fff", cursor: "pointer", color: "#6b7280", fontFamily: "inherit", fontWeight: 700 }}>
                {loading ? "⟳ 로딩중…" : "↻ 새로고침"}
              </button>
            </div>
            <p style={{ margin: "0 0 16px", fontSize: 11, color: "#9ca3af" }}>
              {today} {lastUpdated && `· ${usingFallback ? "로컬 데이터" : "노션 연동"} · ${lastUpdated.toLocaleTimeString("ko-KR")}`}
            </p>
            {!loading && (
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 12, padding: "12px 18px", minWidth: 130 }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: "#2563eb", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>✏️ 크리팀</div>
                  <div style={{ fontSize: 26, fontWeight: 900, color: "#2563eb", lineHeight: 1 }}>{criCount}</div>
                  <div style={{ fontSize: 10, color: "#93c5fd", marginTop: 4 }}>캐드 · 출력 · 은샘플테스트</div>
                </div>
                <div style={{ background: "#faf5ff", border: "1px solid #e9d5ff", borderRadius: 12, padding: "12px 18px", minWidth: 130 }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: "#9333ea", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>🔧 세공실</div>
                  <div style={{ fontSize: 26, fontWeight: 900, color: "#9333ea", lineHeight: 1 }}>{workshopCount}</div>
                  <div style={{ fontSize: 10, color: "#c4b5fd", marginTop: 4 }}>은샘플제작 · 원본제작 · 최종샘플</div>
                </div>
                <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: "12px 18px", minWidth: 130 }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: "#16a34a", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>🏪 운영팀</div>
                  <div style={{ fontSize: 26, fontWeight: 900, color: "#16a34a", lineHeight: 1 }}>{opsCount}</div>
                  <div style={{ fontSize: 10, color: "#86efac", marginTop: 4 }}>최종완료 · 매장준비 · 매장출시</div>
                </div>
                <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 12, padding: "12px 18px", minWidth: 130 }}>
                  <div style={{ fontSize: 10, fontWeight: 800, color: "#d97706", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>📄 상세페이지 대기</div>
                  <div style={{ fontSize: 26, fontWeight: 900, color: "#d97706", lineHeight: 1 }}>{data.filter(d => d.desc.includes("상세페이지 대기")).length}</div>
                  <div style={{ fontSize: 10, color: "#fcd34d", marginTop: 4 }}>콘텐츠팀 액션 필요</div>
                </div>
                {urgent.length > 0 && (
                  <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: "12px 18px", minWidth: 130 }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: "#dc2626", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>⚠️ 미업데이트</div>
                    <div style={{ fontSize: 26, fontWeight: 900, color: "#dc2626", lineHeight: 1 }}>{urgent.length}</div>
                    <div style={{ fontSize: 10, color: "#fca5a5", marginTop: 4 }}>7일 이상 변동 없음</div>
                  </div>
                )}
              </div>
            )}
        ...
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "20px 24px 60px" }}>
          {loading && (
            <div style={{ textAlign: "center", padding: "80px 0", color: "#9ca3af" }}>
              ...
              <p style={{ marginTop: 12 }}>노션에서 데이터 불러오는 중…</p>
          ...
          {!loading && (
            <>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="제품명 / 내용 검색…"
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  {managers.map(m => (
                    <button key={m} onClick={() => setFilterManager(m)} style={{ padding: "7px 12px", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", background: filterManager === m ? "#2563eb" : "#fff", color: filterManager === m ? "#fff" : "#6b7280", border: `1px solid ${filterManager === m ? "#2563eb" : "#e5e7eb"}` }}>
                      {m}
                    </button>
                  ))}
                </div>
                <button onClick={() => setShowDrop(v => !v)} style={{ padding: "7px 12px", borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", background: showDrop ? "#fef2f2" : "#fff", color: showDrop ? "#dc2626" : "#9ca3af", border: `1px solid ${showDrop ? "#fecaca" : "#e5e7eb"}` }}>
                  🚫 드롭 {showDrop ? "숨기기" : "보기"}
                </button>
              </div>
              ...
              {detailWaiting.length > 0 && (
                <div style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: 12, padding: "16px 20px", marginBottom: 18 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                    <span style={{ fontSize: 16 }}>📄</span>
                    <span style={{ fontSize: 13, fontWeight: 900, color: "#b45309" }}>상세페이지 대기</span>
                    <span style={{ fontSize: 10, background: "#fef3c7", color: "#d97706", border: "1px solid #fde68a", padding: "1px 8px", borderRadius: 99, fontWeight: 700 }}>{detailWaiting.length}개</span>
                    ...
                    {detailWaiting.map((item, i) => {
                      ...
                      return (
                        <div key={i} onClick={() => setSelected(item)}
                          style={{ background: "#fff", border: "1px solid #fde68a", borderLeft: `3px solid ${cfg.color}`, borderRadius: "0 8px 8px 0", padding: "10px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10, transition: "all 0.15s" }}
                          onMouseEnter={e => e.currentTarget.style.background = "#fffbeb"}
                          onMouseLeave={e => e.currentTarget.style.background = "#fff"}
                        >
                          <span style={{ fontSize: 10, fontWeight: 700, color: cfg.color, background: cfg.bg, padding: "2px 7px", borderRadius: 99, whiteSpace: "nowrap" }}>{cfg.icon} {item.stage || "미지정"}</span>
